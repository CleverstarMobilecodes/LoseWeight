...
public class MySwitch extends CompoundButton {
	...
	private Drawable mOffTrackDrawable;
    ...
    public MySwitch(Context context, AttributeSet attrs, int defStyle) {
        ...
        mThumbOffDrawable = a.getDrawable(R.styleable.MySwitch_thumbOff);
        
        mTrackDrawable = a.getDrawable(R.styleable.MySwitch_trackSwitch);
        mOffTrackDrawable = a.getDrawable(R.styleable.MySwitch_trackOff);

        // REPLACED
        mTextOn = a.getText(R.styleable.MySwitch_textOff);
        mTextOff = a.getText(R.styleable.MySwitch_textOn);
        ...
        mThumbTextPadding = a.getDimensionPixelSize( R.styleable.MySwitch_thumbTextPaddingSwitch, 0);
        mTrackTextPadding = a.getDimensionPixelSize( R.styleable.MySwitch_trackTextPadding, 0);
        mSwitchMinWidth = a.getDimensionPixelSize( R.styleable.MySwitch_switchMinWidth1, 0);
        mSwitchMinHeight = a.getDimensionPixelSize( R.styleable.MySwitch_switchMinHeight, 0);
        mSwitchPadding =  a.getDimensionPixelSize( R.styleable.MySwitch_switchPadding1, 0);
    ...
    public void setSwitchTextAppearance(Context context, int resid) {
        ...
        //colors = appearance.getColorStateList(R.styleable.mySwitchTextAppearanceAttrib_SwitchTextColor);
        colors = null;
        if (colors != null) {
            mTextColors = colors;
        } else {
        ...
        ts = getResources().getDimensionPixelSize(R.dimen.switchTextSize);
        if (ts != 0) {
            ...
            case MotionEvent.ACTION_MOVE: {
                switch (mTouchMode) {
                    ...
                    case TOUCH_MODE_DRAGGING: {
                    	...
                    	if (mOrientation == HORIZONTAL) {
                            int switchPadding = (int) getResources().getDimension(R.dimen.switchThumbPadding);
                            float newPos = Math.max(switchPadding/2,
			...
			if (!fixed) {
				...
				if (this.mOnChangeAttemptListener != null){
					new Handler().postDelayed(new Runnable() {
						@Override
						public void run() {
							MySwitch.this.mOnChangeAttemptListener.onChangeAttempted(isChecked());							
						}
					}, 300);
				}
				// Log.d(TAG,
    ...
    */
        ...
        if (commitChange) {
            ...
            if (this.mOnChangeAttemptListener != null) {
 				new Handler().postDelayed(new Runnable() {
 					@Override
 					public void run() {
 						MySwitch.this.mOnChangeAttemptListener.onChangeAttempted(newState);							
 					}
 				}, 300);
 			}
    ...
    public void setChecked(boolean checked) {
        ...
        mThumbPosition = lc ? getThumbScrollRange() : switchPadding/2;
        invalidate();
    }
    /*
    ...
    */
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        switchHeight = (int) getResources().getDimension(R.dimen.switchHeight);
        ...
        mSwitchWidth = (int) getResources().getDimension(R.dimen.switchWidth);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (this.mTextOnThumb) {
            mThumbPosition = isChecked() ? getThumbScrollRange() : switchPadding/2;
        } else  {
            mThumbPosition = isChecked() ?  getThumbScrollRange() : switchPadding/2  ;
        }
        ...
        mOffTrackDrawable.setBounds(mSwitchLeft, mSwitchTop, mSwitchRight, mSwitchBottom);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mOrientation == HORIZONTAL) {
        	...
        	if (mPushStyle) {
        		...
        		backingLayer.translate((maxTextWidth - mOffLayout.getWidth())/2 + switchPadding, switchVerticalMid - mOffLayout.getHeight() / 2);
        		mOffLayout.draw(backingLayer);
        		if (mDrawableOff != null) mDrawableOff.draw(backingLayer);
        		backingLayer.restore();
        		backingLayer.translate(maxTextWidth+
        				(maxTextWidth - mOnLayout.getWidth())/2 +
        				mThumbWidth * 2 - switchPadding/2,//+ mThPad.left + mThPad.right,
        				switchVerticalMid - mOnLayout.getHeight() / 2);
        		mOnLayout.draw(backingLayer); 
            ...
            int edgePadding = (int) Constants.pxFromDp(getContext(),3);
        	
        	mThumbDrawable.setBounds(thumbBoundL, mSwitchTop +  edgePadding, thumbBoundR, mSwitchBottom - edgePadding);
        	mThumbOffDrawable.setBounds(thumbBoundL, mSwitchTop +  edgePadding, thumbBoundR, mSwitchBottom - edgePadding);

        	if (getTargetCheckedState()) {
        		mThumbDrawable.draw(canvas);
    		}
    		else {
    			mThumbOffDrawable.draw(canvas);
    		}
        	//mThumbDrawable.draw(canvas);
    ...
    private int getThumbScrollRange() {
        ...
        if (mOrientation == HORIZONTAL){
            int switchPadding = (int) getResources().getDimension(R.dimen.switchThumbPadding);
            range = mSwitchWidth - mThumbWidth - switchPadding;
        }
        if (this.mPushStyle)
